...
def open_trade(conn, symbol, side, entry, sl, tp, risk_pct, start_balance,
    ...
    # --- سقف سهم هر پوزیشن از کل سرمایه (برای تنوع) + سقف سرمایه‌ی واقعاً آزاد ---
    position_cap_dollars = balance * (position_pct_cap / 100)
    margin_cap = min(available_margin, position_cap_dollars)
